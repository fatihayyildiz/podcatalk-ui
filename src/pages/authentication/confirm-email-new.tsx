...
import { useConfirmEmailNewMutation } from "store/services/authApi";

const PageConfirmEmailNew = () => {
    const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const hash = useMemo(() => searchParams.get("hash")?.trim() || "", [searchParams]);

  const [confirmEmailNew, { isLoading, isSuccess, error }] = useConfirmEmailNewMutation();
  ...
  useEffect(() => {
    ...
    confirmEmailNew({ hash });
    localStorage.removeItem('token');    
  }, [hash, confirmEmailNew]);
      ...
      <div className="max-w-md mx-auto space-y-6 text-center">
        ...
        {hash && !isLoading && !isSuccess && error && (
          <div className="space-y-4">
            ...
            <div>
              <ButtonPrimary type="button" onClick={() => confirmEmailNew({ hash })}>
                Try again
              </ButtonPrimary>
            </div>
          </div>
...
export default PageConfirmEmailNew;
